...
# -----------------------------------------------------------------------------
    ...
    def update_matter_curvature(self):
        ...
        if len(curvatures) == len(matter_arr):
            ...
            matter_arr = 0.1 * np.abs(curv_arr) + \
                0.01 * np.random.randn(len(curv_arr))
        ...
        for eps in epsilons:
            if Z.shape[0] >= eps and Z.shape[1] >= eps:
                ...
                reduced = block_reduce(
                    cropped, block_size=(eps, eps), func=np.mean)
    ...
    def analyze_causal_net(self):
        ...
        for i in self.causal_net.nodes:
            for j in self.causal_net.nodes:
                if i != j and nx.has_path(self.causal_net, i, j):
                    ...
                    path_lengths.append(nx.shortest_path_length(
                        self.causal_net, i, j, weight='dt'))
        ...
        print(
            f"Causally connected pairs: {causally_connected}/{n_nodes*(n_nodes-1)}")
        ...
        print(
            f"- Middle event (index {n_nodes//2}) connects to {degrees[n_nodes//2]} future events")
    ...
    def print_physics_insights(self):
        ...
        if len(self.marks) > 1:
            ...
            if avg_position > 0:
                print(
                    f"- Fluctuation/Structure Ratio: {quantum_fluct/avg_position:.6f}")
        else:
        ...
        if hasattr(self, 'avg_causal_path') and hasattr(self, 'matter_degree_corr'):
            ...
            print(
                f"- Max Causal Horizon: {self.marks[-1] - self.marks[0]} temporal units")
            print(
                f"- Matter-Degree Correlation: {self.matter_degree_corr:.3f} (Expected: 0.3-0.7)")
        else:
        ...
        if len(self.curvatures) > 0:
            ...
            print(
                f"- Matter/Curvature Ratio: {energy_balance:.4f} (Expect ~constant)")
        else:
    ...
    def plot_spacetime(self, size=256):
        ...
        x, y, r, theta = self.embed_polar()
        sc1 = ax1.scatter(x, y, c=r, cmap='viridis', s=30,
        ...
        fig.canvas.manager.set_window_title(
            'Quantum Golomb Spacetime Analysis')
